...
<section class="">
	...
	<div class="flex flex-col items-stretch h-full max-w-xl mx-auto">
     ...
     <p class="h-full nieuwssubtitel"> {{ subtitle }}  <!---{{ this.url }} {{ url }} {{ url() }}  method ---></p>
...
<a target="_blank" :href="publicPath + this.$route.query.url" class="block px-4 py-1 m-6 mx-auto text-base font-medium text-center text-white break-all transition-colors duration-500 transform bg-gray-400 border border-gray-600 rounded md:w-64 hover:bg-gray-500 md:mt-0"> &raquo; Test {{ $route.query.url }}  </a>
</keep-alive>
